...
export default function Precios(){
  ...
  // SOLO category === "precios"
  const filtered = useMemo(()=>{
    const s=q.trim().toLowerCase();
    const base = docs.filter(d => String(d.category||"").toLowerCase()==="precios" && (d.active ?? true));
    if(!s) return base;
  ...
  const openOnline = async (doc)=>{
    const sref = ref(storage, doc.storagePath || `precios/${doc.fileName || doc.title}`);
  ...
  const downloadAndShare = async (doc)=>{
    try{
      ...
      let name = sanitize(metaName || doc.fileName || doc.title || "lista-precios");
  ...
  const Thumb = ({uri})=>(
    uri
      ? <Image source={{uri}} style={{ width:128, height:168, borderRadius:12, backgroundColor:"#EEF2F7" }} />
      : <View style={{ width:128, height:168, borderRadius:12, backgroundColor:"#EEF2F7", alignItems:"center", justifyContent:"center" }}>
  ...
  return (
    <Screen>
      <TextInput
        placeholder="Buscar lista o tag..."
